...
10. Decision 19 reports only SENSOR_UNAVAILABLE for an unusable reading, so a checkpoint whose hub was unreachable and whose sps30 was also dead reports one gap, not two. The reasoning is 4.4's both-directions rule: the PDF shows one "Sensor unavailable" line, so one gap is what matches it. Confirm that is wanted, or whether the PDF should list the dead sub-devices underneath and the manifest carry a gap for each.
